  ...
  return showDelayed ? (
    <div
      className={classNames(
        'fixed top-0 h-screen w-full z-40',
        show ? styles.saleModal_fadeUp : styles.saleModal_fadeDown,
      )}
    >
      <div className="h-full w-full bg-ui-black100 opacity-40" />
      <div
        className={classNames(
          '-mt-screenH h-full w-full flex flex-row justify-center items-end md:items-center md:px-8',
        )}
      >
        <div
          className={classNames(
            'bg-ui-popupBg px-6 md:pl-12 lg:pl-16 rounded-t-3xl md:rounded-2xl flex flex-col justify-end md:flex-row md:justify-between lg:justify-start items-center',
            ' md:w-full lg:w-auto relative h-4/5 md:h-512px lg:h-620px',
            styles.saleModal_popup,
            down ? styles.saleModal_goDown : styles.saleModal_goUp,
          )}
        >
          ...
          <button
            className={classNames(
              'bg-ui-white md:bg-none rounded absolute top-8 right-8',
              styles.saleModal_closeButton,
            )}
            onClick={goDown}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 w-6 text-ui-black90"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <div
            className={classNames(
              'md:w-424px lg:w-512px',
              styles.saleModal_modalText,
            )}
          >
            ...
            <h3 className="font-TangoSans md:font-Poppins text-sm-h2-tangosans md:text-md-h3-poppins lg:text-lg-h3-poppins text-ui-black100 font-bold mb-8 md:mb-14 lg:mb-22">
              {t(
                'Do not miss the opportunity to get 100€ discount for your 1st month for our services',
              )}
            </h3>
            <div className="flex flex-col md:flex-row justify-start items-center font-Poppins text-sm-button-md md:text-md-button-md lg:text-lg-button-md">
              <Link href="/project-request" className="w-full md:w-auto">
                <button
              ...
              <a href="tel:+37368356821" className="w-full md:w-auto">
                <button
                  ...
                  onClick={close}
                >
                  <span className="hidden md:block">{t('or')}</span>{' '}
                  <span className="text-ui-pink md:underline">
                    {t('Make a call')}
                  </span>
                </button>
              </a>
          ...
          <div
            className={classNames(
              'hidden md:block md:w-348px lg:w-480px md:h-512px lg:h-620px top-0 right-0 md:absolute lg:relative',
              styles.saleModal_piggyBank,
            )}
          >
            <Image
              src="/popup/piggyBank2.png"
        ...
        {showMiniPopup && (
          <div
            className={classNames(
              'w-full px-container-sm flex flex-row justify-center items-center',
              styles.saleModal_miniPopup,
            )}
          >
            <div
              className={classNames(
                'pt-16 pb-10 px-9 bg-ui-popupBg rounded-2xl relative max-w-576px w-full',
              )}
            >
              <button
                className={classNames(
                  'bg-ui-white md:bg-none rounded absolute top-6 right-6',
                  styles.saleModal_closeButton,
                )}
                onClick={close}
              >
