...
const Header = () => {
  ...
  return(
    <div>
      <div className={classNames(`w-full md:h-18 rounded fixed ${show ? '-top-18' : 'top-0'} z-50 font-Poppins transition-all duration-500`,
              ...
              {routes.map((route, index) =>
                (
                  <Link href={route.link} key={index}>
                    <a>
                      <li key={index} className={`${router.pathname === route.link ? 'md:bg-ui-black90' : ''} px-2 py-2.5 ml-6 lg:ml-10 rounded md:hover:bg-ui-black90 transition duration-300 group mb-6 md:mb-0`}>
